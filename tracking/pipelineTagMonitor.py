...
class MonitorPipeline(object):
  ...
  tracker_added = 0
  tracker_dup = 0
  target_channel = "undefine"
  static_tags = []
  static_tags_str = ""

  def close_spider(self, spider):
    logging.info("spider[%s] is closing..." % spider.source)
    if not isinstance(spider, SpiderBase):
      raise Exception("Spider must extend class <SpiderBase>")
    self.plan_api = spider.settings['PLAN_URL']
    self.tracker_api = spider.settings['TRACKING_URL']
    self.target_channel = spider.target_channel
    self.static_tags = spider.static_tags
    self.static_tags.append(self.target_channel)
    self.static_tags_str = self.list2tagstr(self.static_tags)
    self.plan_api_get = self.plan_api + "&tags=" + self.static_tags_str
    logging.info('------------------------ plan_api %s' % self.plan_api)
    logging.info('------------------------ plan_api_get %s' % self.plan_api_get)
  ...
  def add_plan(self):
    for day in self.url_per_day:
      ...
      for i in range(0, leak):
        ...
        plan["tags"] = self.static_tags
        plan["publishTime"] = int(url_sort[i]["release"]) * 1000 + i
        body = json.dumps(plan)
        self.post_plan(body)

  def get_plan(self):
    req = urllib2.Request(self.plan_api_get)
